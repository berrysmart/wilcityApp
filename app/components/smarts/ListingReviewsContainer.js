...
class ListingReviewsContainer extends Component {
  ...
  _handleDeleteReview = (listingID, reviewID) => async () => {
    ...
    await this.props.deleteReview(listingID, reviewID, listingReviews.total);
        ...
        <View style={styles.totalReviews}>
          <View style={{ flexDirection: "row" }}>
            <Text
              style={[
                stylesBase.text,
                { color: this.props.colorPrimary, fontSize: 18 }
              ]}
            >
              {total}
            </Text>
            <Text style={[stylesBase.text, { fontSize: 16 }]}>{` ${
              translations.reviewsFor
            } ${name}`}</Text>
          </View>
        </View>
      )
