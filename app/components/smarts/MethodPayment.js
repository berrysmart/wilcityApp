...
class MethodPayment extends PureComponent {
  ...
  _renderRadio = () => {
    ...
    const radios = payment.method.map(item => {
      return {
        ...
        value: item.settings.title ? item.settings.title.value : ""
      };
    });
  ...
  _handleClick = async () => {
    ...
    const data = {
      ...
      payment_method_title: payment.methodResult.payment_method_title,
      set_paid: true,
  ...
  _handleNextPage = async () => {
    const { order, getTempToken, onChangePage, auth, payment } = this.props;
    if (!isEmpty(order.orders) && order.statusOrder.status === "success") {
      ...
      Alert.alert("Order create error");
      this.setState({
        loadData: false
      });
    }
