...
export default class Variantions extends PureComponent {
  constructor(props) {
    super(props);
    const { product } = props;
    const { oVariations, oAttributes } = product.variations;
  ...
  componentDidMount() {
    ...
    // setTimeout(() => {
    //   if (this._viewRef) {
    //     this._viewRef.measure(console.log);
    //   }
    // }, 1000);
  }
              ...
              ...(_index !== 0
                ? {
                    [key]: !item.oAttributes[key]
                      ? product.variations.oAttributes[key].options.map(
                          option => ({
                            name: option,
                            isActive: false
                          })
  ...
  _handleSubmit = async () => {
    ...
    const { product, onCart, translations } = this.props;
    const { aVariationGroups } = mapVariations(product.variations.oVariations);
    if (!isEmpty(product.details.variations)) {
      if (
  ...
  _renderPrice = () => {
    const { result } = this.state;
    const { product, colorPrimary } = this.props;
    const { aVariationGroups } = mapVariations(product.variations.oVariations);
    const isPrice =
      Object.keys(result).length === Object.keys(aVariationGroups[0]).length;
    return (
  ...
  _renderContentModal = () => {
    const { oAttributes, result } = this.state;
    const { product } = this.props;
